...
@Component({
  ...
  templateUrl: './flow-background.html',
  styleUrl: './flow-background.scss',
...
export class FlowBackground implements OnInit, IFBackgroundPattern {
  ...
  private readonly _circlePattern =
    viewChild<ElementRef<SVGPatternElement>>('circlePattern');
  private readonly _circlePatternCircle =
    viewChild<ElementRef<SVGCircleElement>>('circle');

  private readonly _rectPattern =
    viewChild<ElementRef<SVGPatternElement>>('rectPattern');
  ...
  private _calculatePattern(): void {
    this._position.x =
      this._transform.position.x + this._transform.scaledPosition.x;
    this._position.y =
      this._transform.position.y + this._transform.scaledPosition.y;
  ...
  private _redrawRectPattern(): void {
    ...
    this._rectPattern()?.nativeElement.setAttribute(
      'width',
      `${this._size.width}`
    );
    this._rectPattern()?.nativeElement.setAttribute(
  ...
  private _redrawCirclePattern(): void {
    this._circlePattern()?.nativeElement.setAttribute(
      'width',
      `${this._radius}`
    );
    this._circlePattern()?.nativeElement.setAttribute(
  ...
  private _redrawCircleElement(): void {
    this._circlePatternCircle()?.nativeElement.setAttribute(
      'r',
