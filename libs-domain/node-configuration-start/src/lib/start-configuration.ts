...
@Component({
  selector: 'start-configuration',
  templateUrl: './start-configuration.html',
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => StartConfiguration),
      multi: true,
    },
    {
  ...
  implements OnInit, ControlValueAccessor, Validator
{
  ...
  private _buildForm(): FormGroup<IConfigurationForm> {
    return this._fb.group<IConfigurationForm>({
      incomingData: this._fb.control(null, [Validators.required]),
    });
  }
