...
@Component({
  selector: 'logger-configuration',
  templateUrl: './logger-configuration.html',
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => LoggerConfiguration),
      multi: true,
    },
    {
  ...
  implements OnInit, ControlValueAccessor, Validator
{
  ...
  private _buildForm(): FormGroup<IConfigurationForm> {
    return this._fb.group<IConfigurationForm>({
      output: this._fb.control(null, [Validators.required]),
    });
  }
